...
addsi *addsi_new(int tableSize)
{
    ...
    // setting start values for harmonics
    ...
    float stepSizeEnv = (M_PI*2) / (float)tableSize;
    float currentXEnv = 0;
    
    for(int i = 0; i < x->tableSize; i++)
    {
        x->lookupTable[i] = sinf(currentXEnv);
        x->envelopeTable[i] = 1;
        currentXEnv += stepSizeEnv;
    }
...
void addsi_process(addsi *x, float *in, float *out, int vectorSize)
{
    ...
    while(i--)
    {
        ...
        for(int i = 0; i < x->numberOfHarmonics; i++)
        {
            ...
            buff += x->lookupTable[intIndex] * (x->harmonicGain[i] * LFO2osc) ;
